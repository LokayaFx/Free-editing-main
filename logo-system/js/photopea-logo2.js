/**
 * Photopea API integration for Logo 2 (all characters)
...
class PhotopeaLogoRenderer {
    ...
    init() {
        return new Promise((resolve) => {
            if (this.iframe) {
                // If it already exists, just resolve if ready
                if (this.appReady) return resolve(true);
                // Otherwise wait for the message listener
            }
...
async function renderLogo(userData) {
    try {
        console.log('=== LOGO 2 GENERATION START ===', userData);
        ...
        const psdUrl = `../logo2-char/char${charN}/psd/s2_c${charN}.psd`;
        ...
        a.href     = url;
        a.download = `lokaya-logo2-c${charN}.png`;
        ...
        console.log('=== LOGO 2 GENERATION DONE ===');
        return true;
    } catch (err) {
        console.error('=== LOGO 2 GENERATION FAILED ===', err);
        return false;
    }
}
